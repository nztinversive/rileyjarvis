...
The iOS app targets iOS 15+ with Capacitor 8 and Xcode 26+. Native iOS uses a mobile-first Talk, Artifacts, and Activity tab model; Computer mode and Remote Codex remain desktop-only. Simulator setup, viewport QA, secure bootstrap provisioning boundaries, and the limits of Phase 4 validation are documented in [docs/ios.md](docs/ios.md). Physical-iPhone voice remains unproven until Phase 5.
