...
Bootstrap bearer tokens currently map to server-owned opaque subjects. That is deliberately narrower than a final account system and must be replaced at the authentication interface by Sign in with Apple or another identity provider in a later phase. The runtime bridge provides Keychain `get` and `delete` only; a launch-argument-gated native provisioning sheet is compiled only in DEBUG and writes directly to `WhenUnlockedThisDeviceOnly` storage. There is no Preferences fallback or Release credential-write path. Phase 5 verifies this boundary and the audio lifecycle through tests and Simulator builds; physical microphone input, audible output, accessory routing, real interruptions, signing, and device behavior remain unverified and are deferred to Phase 8 or a dedicated pre-release hardware gate. Subscriptions, feature/data sync, and remote tool execution remain deferred.
